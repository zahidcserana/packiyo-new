<div class="modal fade confirm-dialog" id="import-location-type-modal" role="dialog">
    <div class="modal-dialog modal-md modal-dialog-centered" role="document">
        <div class="modal-content bg-white">
            <form method="post" action="{{ route('location_type.importCsv') }}" autocomplete="off" class="import-location-type-form modal-content">
                @csrf
                <div class="modal-header border-bottom mx-4 px-0">
                    <h6 class="modal-title text-black text-left" id="modal-title-notification">{{ __('Import Location Types From CSV') }}</h6>
                        ...
                        <div class="w-100">
                            <div class="form-group mb-0 mx-2 text-left mb-3">
                                <div class="">
                                                ...
                                                <tr>
                                                    <td style="white-space: unset">
                                                        <div class="form-group mx-2 text-center">
                                                            ...
                                                            <div class="input-group input-group-alternative input-group-merge bg-lightGrey font-sm">
                                                                <input
                                                                    class="form-control font-sm bg-lightGrey font-weight-600 text-neutral-gray h-auto p-2"
                                                                    placeholder="{{ __('Upload CSV file') }}"
                                                                    type="file"
                                                                    name="import_csv"
                                                                    id="location-type-csv-button"
                                                                    accept=".csv"
                                                                    style="display: none"
                                                                >
                                                            </div>
                                                            <button
                                                                onclick="document.getElementById('location-type-csv-button').click()"
                                    ...
                                    <div id="csv-filename" class="d-flex justify-content-center"></div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn bg-logoOrange mx-auto px-5 text-white import-location-type">{{ __('Import') }}</button>
                </div>
            </form>
        </div>
    </div>
