@if(request()->has('customize'))
    <select class="widget-select form-control form-control-sm" style="width: fit-content; display: inline-block;">
        @foreach(\App\Models\UserWidget::WIDGET_LIST as $shortcode => $data)
            <option value="{{str_replace(['[',']'], '', $shortcode)}}">{{$data['title']}}</option>
        @endforeach
    </select>
    <button class="btn btn-sm btn-primary new-widget grid-button">{{__('Add widget')}}</button>
    <br>
    <button class="btn btn-sm btn-primary save-grid grid-button">{{__('Save layout')}}</button>
    <button class="btn btn-sm btn-primary static grid-button">{{__('Static')}}</button>
    <button class="btn btn-sm btn-primary recalculate grid-button">{{__('Recalculate Height')}}</button>
@endif
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('body').on('gridstack-loaded', function () {
            setTimeout(function () {
                RevenueChart();
                WidgetOrdersReceived();
                WidgetOrdersShipped();
                WidgetReturns();
                WidgetPurchaseOrdersComingIn();
                WidgetPurchaseOrdersReceivedQuantity();
                ...
                */
                ...
                $.each($('.grid-stack-item'), function(key,value){
                    ...
                    var className = attrValue.substring(attrValue.indexOf('[') + 1, attrValue.indexOf(']'));
