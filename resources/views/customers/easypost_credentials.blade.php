                    ...
                    <div class="card-body">
                        <div class="nav-wrapper">
                            <ul class="nav nav-pills nav-fill flex-column flex-md-row" id="tabs-icons-text" role="tablist">
                                ...
                                <li class="nav-item">
                                    <div class="nav-link mb-sm-3 mb-md-0 active" id="tabs-icons-text-3-tab" aria-controls="tabs-icons-text-3" aria-selected="true"><i class="ni ni-cloud-upload-96 mr-2"></i>{{__('Easypost Credentials')}}</div>
                                </li>
                                <li class="nav-item">
                        ...
                        <div class="card shadow">
                            <div class="card-body">
                                <div class="table-responsive table-overflow items-table mb-4">
                                    <table class="col-12 table align-items-center table-flush">
                                        ...
                                        <tr>
                                            ...
                                            <th scope="col">{{ __('Test API Key') }}</th>
                                            <th scope="col"></th>
                                        </tr>
                                        </thead>
                                        <tbody id="item_container">
                                        @foreach($customer->easypostCredentials as $easypostCredential)
                                            <tr>
                                                <td>
                                                    {{ $easypostCredential->api_key }}
                                                </td>
                                                <td>
                                                    {{ $easypostCredential->test_api_key }}
                                                </td>
                                                <td>
                                                    <a href="{{ route('customers.easypost_credentials.edit', ['customer' => $customer, 'easypost_credential' => $easypostCredential]) }}" class="table-icon-button">
                                                        <i class="picon-edit-filled icon-orange icon-lg" title="{{ __('Edit') }}"></i>
                                                    </a>
                                                    <form action="{{ route('customers.easypost_credentials.destroy', ['customer' => $customer, 'easypost_credential' => $easypostCredential]) }}" method="post" class="d-inline-block">
                                                        <input type="hidden" name="_method" value="delete">
                                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                        <input type="hidden" name="id" value="{{ $easypostCredential->id }}">
                                ...
                                <div class="text-right">
                                    <a href="{{ route('customers.easypost_credentials.create', compact('customer')) }}" class="btn bg-logoOrange text-white my-2 px-3 py-2 font-weight-700 border-8">{{ __('Add new credentials') }}</a>
                                </div>
                            </div>
                        </div>
                    </div>
