@3pl @billing @invoice
Feature: Generate invoices for 3pl clients without wallet feature
    As the owner of a 3PL business
    I want to generate invoices for my client without wallet feature
    ...
    Background:
        ...
        And the customer "Test 3PL Client 2" has an SKU "test-product-blue" named "Test Product Blue" weighing 3.99
        And the customer "Test 3PL Client 2" has an SKU "test-product-red" named "Test Product Red" weighing 5.99
        And the customer "Test 3PL Client 2" has an SKU "test-product-yellow" named "Test Product Yellow" weighing 5.99
        And the customer "Test 3PL Client" has a shipping box named '6" x 6" x 6" Brown Box'
        And the customer "Test 3PL Client 2" has a shipping box named '6" x 6" x 6" Brown Box'
        And the customer "Test 3PL" has a shipping box named '6" x 6" x 6" Brown Box' with cost "10.00"
        ...
        And I manually set 100 of "test-product-red" into "A-1" location
        And I manually set 100 of "test-product-yellow" into "A-1" location
        And I will work with customer "Test 3PL Client 2"
        And I manually set 100 of "test-product-blue" into "A-1" location
        And I manually set 100 of "test-product-red" into "A-1" location
        And I manually set 100 of "test-product-yellow" into "A-1" location

    Scenario: When generating batch invoices for multiple clients, generation is successful.
        ...
        And a shipping label rate "Test Shipping Label Rate" on rate card "Test Rate Card 2"
        And the shipping label rate "Test Shipping Label Rate" applies when other rate matches
        And the shipping label rate has a flat fee of "1.50"
        And a package rate "Test Package Rate" on rate card "Test Rate Card 2"
        And the package rate "Test Package Rate" has a flat fee of "3.50"
        And a picking rate "Test Picking Rate" on rate card "Test Rate Card 2"
        And the picking rate "Test Picking Rate" has a flat fee of "0.5"
        ...
        And the customer "Test 3PL Client 2" got the order number "O-004" for 1 SKU "test-product-blue"
        And the customer "Test 3PL Client 2" got the order number "O-005" for 1 SKU "test-product-red"
        And the customer "Test 3PL Client 2" got the order number "O-006" for 1 SKU "test-product-yellow"
        And the user "roger+test_3pl@packiyo.com" is authenticated
        ...
        And the 3PL "Test 3PL" ships order "O-004" for client "Test 3PL Client 2" through "FedEx" on the "2024-05-02" with tracking "TN-O-004" and cost "1.00"
        And the 3PL "Test 3PL" ships order "O-005" for client "Test 3PL Client 2" through "FedEx" on the "2024-05-02" with tracking "TN-O-005" and cost "1.00"
        And the 3PL "Test 3PL" ships order "O-006" for client "Test 3PL Client 2" through "FedEx" on the "2024-05-02" with tracking "TN-O-006" and cost "1.00"
        ...
        And invoices are calculated in the background with no wallet feature
        Then a new invoice is generated for 3pl client "Test 3PL Client" for the period "2024-05-01" to "2024-05-15" with status "done"
        And a new invoice is generated for 3pl client "Test 3PL Client 2" for the period "2024-05-01" to "2024-05-15" with status "done"
        And batch bills for "Test 3PL" have status "done"
        And no invoice cache document is generated

    Scenario: When generating batch invoices for multiple clients, but at least one calculation is an error, batch status is set to failed
        Given a shipping label rate "Test Shipping Label Rate" on rate card "Test Rate Card"
        And the shipping label rate "Test Shipping Label Rate" applies when other rate matches
        And the shipping label rate has a flat fee of "1.50"
        And a package rate "Test Package Rate" on rate card "Test Rate Card"
        ...
        And the 3PL "Test 3PL" ships order "O-006" for client "Test 3PL Client 2" through "FedEx" on the "2024-05-02" with tracking "TN-O-006" and cost "1.00"
        When I calculate invoice for 3pl "Test 3PL" clients for the period "2024-05-01" to "2024-05-15"
        And invoices are calculated in the background with no wallet feature only first job is a success
        Then a new invoice is generated with status "failed"
        And bulk invoice for "Test 3PL" batch with status "failed"

    Scenario: When generating batch invoices for multiple clients, but every calculation is an error, batch status is set to failed
        Given a shipping label rate "Test Shipping Label Rate" on rate card "Test Rate Card"
        And the shipping label rate "Test Shipping Label Rate" applies when other rate matches
        And the shipping label rate has a flat fee of "1.50"
        And a package rate "Test Package Rate" on rate card "Test Rate Card"
        ...
        And the 3PL "Test 3PL" ships order "O-006" for client "Test 3PL Client 2" through "FedEx" on the "2024-05-02" with tracking "TN-O-006" and cost "1.00"
        When I calculate invoice for 3pl "Test 3PL" clients for the period "2024-05-01" to "2024-05-15"
        And invoices are calculated in the background with no wallet feature all jobs are a failure
        Then a new invoice is generated with status "failed"
        And bulk invoice for "Test 3PL" batch with status "failed"
